...
class DefaultController extends Controller
{
    ...
    public function indexAction(Request $request)
	{
		...
		if (empty($_GET)) {			 
		    $playerselected = (string) rand(1,18589);
			$sql            = "select playerID from people where id=?";
			$stmt           = $conn->prepare($sql);
			$stmt->bindValue(1, $playerselected);
			$stmt->execute();
		...
		$salary        = $conn->fetchall('select  yearID,     teamID,salary from Salaries a where playerID=?', array($name)); 
		$manager       = $conn->fetchall('select  b.nameLast, b.nameFirst, a.playerID,   a.yearID, a.teamID from Appearances a, ManagersSummary b where a.yearID = b.yearID and a.teamID = b.TeamID and a.playerID = ?', array($name));     
        ...
        return $this->render('baseball/index.html.twig', array('salary' => $salary, 'manager' => $manager, 'hof'=> $hof,'award'=> $award,'appearances' => $appearances,'allstar' => $allstar, 'fielding' => $fielding, 'pitching' => $pitching, 'personal' => $personal, 'batting' => $batting, 'check' => $playerselected));  
